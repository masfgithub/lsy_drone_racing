...
def make_obs() -> SimpleNamespace:
    ...
    centers = np.array([[0.5, 0.25, 0.7], [1.05, 0.75, 1.2], [-1.0, -0.25, 0.7], [0.0, -0.75, 1.2]])
    ...
    obstacles = np.array(
        [[0.0, 0.5, 1.55], [0.75, 0.5, 1.55], [-1.5, -0.25, 1.55], [-0.5, -0.75, 1.55]]
    )
...
def main() -> None:
    ...
    planner = GateCenterSplinePlanner(
        obs, info={}, config=config, t_total=8.0, thread_separation=THREAD_SEPARATION
    )
    ...
    print(
        f"speed range: {np.linalg.norm(traj.velocities, axis=1).min():.2f} "
    ...
    if planner._avoid_log:
        ...
        for d in planner._avoid_log:
            print(
                f"  obstacle {d['obstacle']}: chose '{d['chosen']}'  "
    ...
    final_viol = planner._count_violations(traj.positions[:, :2], obs_xy, planner._obstacle_d_min)
    ...
    if planner._frame_log:
        ...
        for d in planner._frame_log:
            print(
                f"  gate {d['gate']}: re-crossing at offset {d['from_offset']} m "
