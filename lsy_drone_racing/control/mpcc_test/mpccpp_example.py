...
def simulate(n_steps=50):
    ref = TunnelReferencePath(
        ...
        W_nom=3.0, tunnel_sigma=1.2,
    )
    print(f"path length = {ref.length:.2f} m")

    n_obst = len(OBSTACLE_CENTERS)
    cfg = MPCCppConfig(
        use_tunnel=True, tunnel_soft=True,
        use_obstacles=True, n_obstacles=n_obst, obstacle_soft=True,
    )
    ...
    print(f"min distance to a stick = {log['obs_dist'].min():.3f} m "
...
def plot(ref, obstacles, log, save_prefix="mpccpp"):
    ...
    a.set_xlabel("x [m]"); a.set_ylabel("y [m]"); a.set_title("top view + tube + sticks")
    ...
    axs[1, 2].plot(log["t"], log["obs_dist"]); axs[1, 2].axhline(obs_r, color="darkorange", ls="--")
...
def _draw_stick(ax, xy, r, z0, z1, color="darkorange"):
