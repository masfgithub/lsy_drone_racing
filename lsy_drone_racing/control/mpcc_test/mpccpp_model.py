...
def num_params(cfg: MPCCConfig) -> int:
    """Length of the parameter vector p for the given config."""
    n = NP_BASE_TUNNEL
...
def num_h(cfg: MPCCConfig) -> int:
    """Number of nonlinear constraint rows (con_h_expr) for the given config."""
...
class MPCCppConfig(MPCCConfig):
    """Configuration for the MPCC++ model: base MPCC plus tunnel and obstacle options."""
...
def tunnel_constraint(x: ca.SX, p: ca.SX) -> ca.SX:
...
def obstacle_constraint(x: ca.SX, p: ca.SX, n_obstacles: int) -> ca.SX:
...
def export_mpccpp_model(cfg: MPCCppConfig) -> AcadosModel:
    """Build the acados model for the MPCC++ quadrotor: dynamics, cost, and constraints."""
    npar = num_params(cfg)
    x = ca.SX.sym("x", NX)
    u = ca.SX.sym("u", NU)
    p = ca.SX.sym("p", npar)
