"""Builds the acados OCP for the MPCC formulation and runs the receding-horizon loop.

Algorithm 1 of the paper. See mpcc_model.py for the state convention.
...
class MPCCController:
    """MPCC controller: builds the acados OCP and runs the receding-horizon loop."""

    def __init__(
        self,
        cfg: MPCCConfig,
        reference: ReferencePath,
        json_file: str = "acados_ocp_mpcc.json",
        build: bool = True,
    ):
        """Set up the reference and (optionally) build the acados OCP solver."""
        self.cfg = cfg
        self.ref = reference
        self.N = cfg.N
        self.dt = cfg.dt
    ...
    def _build(self, json_file: str) -> None:
        """Build the acados OCP (cost, constraints) and instantiate the solver."""
        cfg = self.cfg
        ocp = AcadosOcp()
    ...
    # ----------------------------------------------------- initial full state
    def initial_state(
        self, p: np.ndarray, q: np.ndarray, v: np.ndarray, w: np.ndarray, vtheta: float = 1.0
    ) -> np.ndarray:
        """Build the full initial state x0, projecting p onto the reference path for theta."""
        x = np.zeros(NX)
        x[0:3] = p
    ...
    def feedback_state(
        self, p: np.ndarray, q: np.ndarray, v: np.ndarray, w: np.ndarray
    ) -> np.ndarray:
        """Assemble the next x0 from measured [p,q,v,w] and the carried-over virtual states."""
        x = np.zeros(NX)
        x[0:3] = p
        x[3:7] = self._norm_quat(q)
        x[7:10] = v
    ...
    # --------------------------------------------------------------- warm start
    def _init_warmstart(self, x0: np.ndarray) -> None:
        """Initialize the solver's state/input trajectories from a constant-speed guess."""
    ...
    def _nominal_state(self, theta: float, vth: float) -> np.ndarray:
        """Nominal full state at path position theta moving at speed vth."""
        x = np.zeros(NX)
        x[3] = 1.0
    ...
    # --------------------------------------------------------------- main solve
    def solve(self, x: np.ndarray) -> dict:
        """Set x0 and per-node parameters, solve the OCP, and return the command dict."""
        ...
        theta_next = sol_theta[-1] + self.dt * self._vtheta_pred1
        self.theta_pred = np.concatenate([sol_theta[1:], [theta_next]])
        ...
        return dict(
            ...
            theta=self._theta_pred1,
            vtheta=self._vtheta_pred1,
        )

    # --------------------------------------------------------------- utilities
    @staticmethod
    def _norm_quat(q: np.ndarray) -> np.ndarray:
        """Normalize a quaternion, falling back to identity if it is near zero."""
