...
def build_ocp(obst: np.ndarray, d_min: float, N: int = 20, Tf: float = 2.0) -> AcadosOcp:
    ...
    ocp.constraints.lbx = np.array([-20.0, -20.0, -5.0, -5.0])
    ocp.constraints.ubx = np.array([20.0, 20.0, 5.0, 5.0])
...
def plot_results(
    ...
    """Three-panel plot: 2-D trajectory | velocity time-series | control inputs."""
    N = u_traj.shape[0]
    t = np.linspace(0, Tf, N + 1)
    tc = np.linspace(0, Tf, N)  # control knots (one fewer than states)
    ...
    for i, obs in enumerate(obstacles):
        # Filled safety circle
        ax.add_patch(plt.Circle(obs, d_min, color="tab:red", alpha=0.20, zorder=2))
        # Dashed boundary
        ax.add_patch(
            plt.Circle(
    ...
    for k in range(n_seg):
        ...
        ax.plot(x_traj[k : k + 2, 0], x_traj[k : k + 2, 1], color=c, linewidth=2.0, zorder=5)
    ax.plot(
        x_traj[:, 0],
        x_traj[:, 1],
        "o",
    ...
    ax.plot(*x0[:2], "gs", markersize=10, label="start", zorder=7)
    ax.plot(*x_ref[:2], "g*", markersize=14, label="goal", zorder=7)
    ...
    all_pts = np.vstack([x_traj[:, :2], obstacles, x0[:2], x_ref[:2]])
    margin = d_min + 0.5
    ...
    ax.plot(t, x_traj[:, 2], label="vx", linewidth=1.8)
    ax.plot(t, x_traj[:, 3], label="vy", linewidth=1.8)
    speed = np.linalg.norm(x_traj[:, 2:4], axis=1)
...
if __name__ == "__main__":
    ...
    x_ref = np.array([8.0, 2.0, 0.0, 0.0])
