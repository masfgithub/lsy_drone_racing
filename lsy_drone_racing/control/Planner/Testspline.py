...
def build_figure(start, gates, obstacles, traj, traj0, waypoints):
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            mode="markers+text",
            marker=dict(
                size=5, color="orange", symbol="diamond", line=dict(color="black", width=1)
            ),
    ...
    fig.update_layout(
        ...
        title="SplinePlanner trajectory + waypoints",
...
def main():
    ...
    obstacles = [[0.0, 0.6, 1.55], [1.0, 0.25, 1.55], [-1.27, -0.4, 1.55], [-0.7, -0.75, 1.55]]
