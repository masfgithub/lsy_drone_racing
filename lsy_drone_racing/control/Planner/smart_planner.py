...
class SplinePlanner(Planner):
    ...
    def _avoidance_tree(
        ...
        p_WLL_array = self._avoid_gates_tree(p_WLL_array, pGLL_array, y_GBL_array, pOLL_array, t_elapsed)
        p_WLL_array = self._avoid_obsticles(p_WLL_array, pOLL_array, pGLL_array, y_GBL_array, t_elapsed)
    ...
    def _pick_better(self, wps_A, clear_A, wps_B, clear_B, t_elapsed, depth=0):
        ...
        indent = "  " * depth
        print(f"{indent}A: clear={clear_A}, gates={gates_A}, len={len_A:.2f}   "
    ...
    def _explore_branch(self,
        ...
        for _ in range(max_iter):
            ...
            target_violation = self._find_specific_obstacle_violation(
                branch_pts, target_obst_c
            )
    ...
    def _compute_gate_detour_waypoint(self, gate_c, gate_yaw, push_vector):
        ...
        Returns:
            new_wp:       3D detour waypoint position.
        """
        push_length = FRAME_WIDTH/2 + 0.2#self._get_gate_push(gate_c.copy(), gate_c, gate_yaw, push_vector)
        new_wp = gate_c + push_length * push_vector
        return new_wp


    def _find_first_gate_violation(self, pts, cum, pGLL_array, y_GBL_array, 
                                skip_approach=True):
    ...
    def _evaluate_gate_branch(self,
                            wps: np.ndarray,
                            new_wp: np.ndarray,
                            s_detour: float,
        ...
        try:
            ...
            return {'wps': wps, 'gate_hits': 999, 'obst_hits': 999,
                    'local_len': float('inf'), 'success': False}
        ...
        for it in range(max_iter):
            try:
                ...
                return {'wps': test_wps, 'gate_hits': 999, 'obst_hits': 999,
                        'local_len': float('inf'), 'success': False}
            ...
            entry_i, exit_i = self._find_target_gate_violation(
                pts, target_gate_c, target_gate_yaw
            )
            if entry_i is None:
            ...
            push_vector_iter = (p_mid_new - target_gate_c)/np.linalg.norm(p_mid_new - target_gate_c)
            ...
            extra_push = self._get_gate_push(p_mid_new, target_gate_c, target_gate_yaw, push_vector_iter)
            push_wp = p_mid_new + extra_push * push_vector_iter
            s_new = 0.5 * (cum[entry_i] + cum[exit_i])
            # NEW DIAGNOSTIC:
            print(f"    [iter {it}] cleanup wp = {push_wp}  "
        ...
        if violation_arclens:
            print(f"    [diag] violations at arc-length "
    ...
    def _avoid_gates_tree(self,
        ...
        for outer_iter in range(_MAX_AVOID_ITER):
            ...
            branch_L = self._evaluate_gate_branch(
                wps, wp_L, s_detour, push_L, gate_c, gate_yaw,
                t_elapsed, pGLL_array, y_GBL_array, pOLL_array,
            )
            branch_R = self._evaluate_gate_branch(
                wps, wp_R, s_detour, push_R, gate_c, gate_yaw,
                t_elapsed, pGLL_array, y_GBL_array, pOLL_array,
            )
            branch_T = self._evaluate_gate_branch(
                wps, wp_T, s_detour, push_T, gate_c, gate_yaw,
                t_elapsed, pGLL_array, y_GBL_array, pOLL_array,
            )
    ...
    def _pick_best_gate_branch(self,
                                branch_L: dict,
                                branch_R: dict,
                                branch_T: dict
                                ) -> tuple[dict, str]:
    ...
    def _plot_gate_branch(self,
        ...
        # ----- 3D view -----
        # Trajectory
        ax3d.plot(pts[:, 0], pts[:, 1], pts[:, 2], "-", color="tab:green", lw=2.0,
                label=f"trajectory ({len(pts)} pts)")
        
        # Violation samples
        if len(violation_pts):
            ax3d.scatter(violation_pts[:, 0], violation_pts[:, 1], violation_pts[:, 2],
                        c="red", s=15, marker="x",
                        label=f"violations ({len(violation_pts)})")
        
        # Waypoints (numbered)
        ax3d.scatter(wps[:, 0], wps[:, 1], wps[:, 2],
                    c="orange", marker="D", s=60, edgecolor="k",
        ...
        self._plot_gate_frame_3d(ax3d, target_gate_c, target_gate_yaw,
                                FRAME_WIDTH / 2, "tab:blue", lw=2.5)
        self._plot_gate_frame_3d(ax3d, target_gate_c, target_gate_yaw,
                                FRAME_OPENING / 2, "tab:cyan", lw=1.5)
        
        # Other gates (faded)
        for k, (gc, gy) in enumerate(zip(pGLL_array, y_GBL_array)):
            if np.allclose(gc, target_gate_c):
                continue
            self._plot_gate_frame_3d(ax3d, gc, gy, FRAME_WIDTH / 2,
                                    "gray", lw=1.0, alpha=0.4)
        
        # Obstacles (just markers in 3D for clarity)
        if len(pOLL_array):
            ax3d.scatter(pOLL_array[:, 0], pOLL_array[:, 1], pOLL_array[:, 2],
                        c="firebrick", s=60, marker="^", alpha=0.7, label="obstacles")
        ...
        # ----- Top-down view (xy) -----
        # Trajectory
        ax_top.plot(pts[:, 0], pts[:, 1], "-", color="tab:green", lw=2.0,
                    label="trajectory")
        
        # Violations
        if len(violation_pts):
            ax_top.scatter(violation_pts[:, 0], violation_pts[:, 1],
                        c="red", s=20, marker="x", label="violations")
        
        # Waypoints
        ax_top.scatter(wps[:, 0], wps[:, 1], c="orange", marker="D", s=70,
                    edgecolor="k", label="waypoints", zorder=5)
        for i, p in enumerate(wps):
            ax_top.annotate(str(i), (p[0], p[1]), textcoords="offset points",
        ...
        ax_top.plot([outer_a[0], outer_b[0]], [outer_a[1], outer_b[1]],
                    color="tab:blue", lw=3, label="gate outer frame")
        # Opening: cyan line through the opening width
        ax_top.plot([open_a[0], open_b[0]], [open_a[1], open_b[1]],
                    color="tab:cyan", lw=4, label="gate opening")
        # Gate center
        ax_top.scatter([g_xy[0]], [g_xy[1]], c="blue", s=80, marker="o",
                    edgecolor="k", zorder=6, label="gate center")
        
        # Obstacles (keep-out shells)
        for o in pOLL_array:
            ax_top.add_patch(plt.Circle((o[0], o[1]), R_OBSTACLE + CLEARANCE,
                                        color="orange", alpha=0.2))
            ax_top.add_patch(plt.Circle((o[0], o[1]), 0.15,
        ...
        for gc, gy in zip(pGLL_array, y_GBL_array):
            ...
            ax_top.plot([oa[0], ob[0]], [oa[1], ob[1]],
                        color="gray", lw=1.0, alpha=0.4)
    ...
    def _plot_gate_frame_3d(self, ax, c, yaw, half, color, lw=2.0, alpha=1.0):
        ...
        pts = np.array([np.asarray(c) + a * half * w + b * half * zz
                        for a, b in corners])
        ax.plot(pts[:, 0], pts[:, 1], pts[:, 2], color=color, lw=lw, alpha=alpha)


    def _plot_full_trajectory(self,
                          wps: np.ndarray,
                          spline,
                          t_dense: np.ndarray,
                          pts: np.ndarray,
        ...
        if len(wps):
            ax3d.scatter(wps[0, 0], wps[0, 1], wps[0, 2],
                        c="black", s=100, marker="o", label="start")
        
        # All gates
        for k, (gc, gy) in enumerate(zip(pGLL_array, y_GBL_array)):
            self._plot_gate_frame_3d(ax3d, gc, gy, FRAME_WIDTH / 2,
                                    "tab:blue", lw=2.0)
            self._plot_gate_frame_3d(ax3d, gc, gy, FRAME_OPENING / 2,
                                    "tab:cyan", lw=1.3)
            ax3d.text(gc[0], gc[1], gc[2] + 0.15, f"G{k}",
                    fontsize=9, color="tab:blue", weight="bold")
        
        # Obstacles
        if len(pOLL_array):
            for j, o in enumerate(pOLL_array):
                ax3d.scatter(o[0], o[1], o[2], c="firebrick",
                            s=60, marker="^", alpha=0.7)
        ...
        # ----- Top-down view -----
        ax_top.plot(pts[:, 0], pts[:, 1], "-",
                    color="tab:green", lw=2.0, label="trajectory")
        ...
        ax_top.scatter(wps[:, 0], wps[:, 1], c="orange", marker="D", s=70,
                    edgecolor="k", label=f"waypoints ({len(wps)})", zorder=5)
        for i, p in enumerate(wps):
            ax_top.annotate(str(i), (p[0], p[1]),
                            textcoords="offset points", xytext=(5, 5), fontsize=7)
        
        if len(wps):
            ax_top.scatter(wps[0, 0], wps[0, 1], c="black", s=100,
        ...
        for k, (gc, gy) in enumerate(zip(pGLL_array, y_GBL_array)):
            ...
            ax_top.plot([open_a[0], open_b[0]], [open_a[1], open_b[1]],
                        color="tab:cyan", lw=3.5)
            ax_top.scatter([g_xy[0]], [g_xy[1]], c="blue", s=60,
                        marker="o", edgecolor="k", zorder=6)
            ax_top.annotate(f"G{k}", (g_xy[0], g_xy[1]),
